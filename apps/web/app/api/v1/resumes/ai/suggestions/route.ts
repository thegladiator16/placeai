...
type Suggestion = { type: 'improvement' | 'warning' | 'tip'; section: string; message: string };
...
export async function POST(req: NextRequest) {
  ...
  const aiRes = await fetch(`${AI_SERVICE_URL}/generate/suggestions`, {
    method: 'POST',
  ...
  if (!aiRes.ok) {
    return NextResponse.json({ success: true, data: [] } satisfies ApiResponse<Suggestion[]>);
  }

  const result = await aiRes.json() as { suggestions: Suggestion[] };
  return NextResponse.json({ success: true, data: result.suggestions } satisfies ApiResponse<Suggestion[]>);
}
